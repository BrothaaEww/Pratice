 ...
 * rats present in an area, ‘unit’ is the amount of food each rat consumes and
 * each ith element of array ‘arr’ represents the amount of food present in
 ...
 * Input:
 * 
 * r: 7 unit: 2 n: 8 arr: 2 8 3 5 7 4 1 2 Output: 4
 * 
...
public class Rats {
	...
	public static int getHouseCount(int r, int unit, int[] arr, int n) {
		if (arr == null)
			return -1;
		int count = 0;
		int requirement = r * unit;
		int sum = 0;
		for (int i = 0; i < n; i++) {
			if (sum >= requirement)
				break;
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < n; i++)
			arr[i] = sc.nextInt();
		sc.close();
		System.out.println(Rats.getHouseCount(r, unit, arr, n));
	}
}
